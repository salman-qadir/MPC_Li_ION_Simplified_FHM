...
%                costr=sum( k*(y.socn-w.socr(w.ij)).^2 + j*u.^2);
%                costt =j*(100-y.socn(1)).^2;
...
function [c,ceq] = constraint(w,da,u)
     ...
     y.v-4.3; -y.v+1.125;(-min(y.cel))+1e-3;-0*fm(y.opns1)];% 4.118
%    cr   = [ -y.socn;y.socn-100;-y.socp;y.socp-100;
...
%      end     
     if (max(d1.v)>4.5)||(min(d1.v)<1.4)||(min(min(d1.cel))<1e-3)||max(abs(d1.socn-99))<.1||max(abs(d1.socp-99))<.1||ij==length(w.iter)-w.N      
