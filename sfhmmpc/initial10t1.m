addpath(genpath('C:\Users\salma\MATLAB Drive\T9_ss_mpc\sfhmmpc') );
% clearvars; close all; clc
%p=param10t1;
% p.c=ab; p.rsei=bc;
 p.cr=2;
p.tme= 0.0;
p.u=0*(((p.cr*1*p.c))/p.a);
p.soci=0;p.socrr=100;
...
p.y0.x=[p.qb;cnn;cpp;bc;ab;0;p.y0.jsn1]';
p.y0.socn=p.soci;p.y0.soh=ab/p.c;p.y0.rfilm=p.rsei;
p.y0.u0= p.u*ones(1,p.M);
% p.y0s=[cnn;cpp];
p.socr = [linspace(p.soci,p.socrr,round(length(p.ts))),p.socrr*ones(1,2e3)];
p.soh=[linspace(1,0.9998,length(p.ts)),0.9998*ones(1,2e3)];
